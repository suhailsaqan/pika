...
if [[ -z "${STATE_DIR:-}" ]]; then
  AUTO_STATE_DIR=1
  STATE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/openclaw-marmot.phase2.XXXXXX")"
fi
RELAY_URL="${RELAY_URL:-}"
...
if [[ -z "${RELAY_URL}" ]]; then
  ...
  if ! wait_for_tcp 127.0.0.1 "${RELAY_PORT}"; then
    echo "[phase2] relay did not start; last logs:" >&2
...
cargo run --manifest-path "${REPO_ROOT}/Cargo.toml" -p marmotd -- scenario invite-and-chat-rust-bot --relay "${RELAY_URL}" --state-dir "${STATE_DIR}"
