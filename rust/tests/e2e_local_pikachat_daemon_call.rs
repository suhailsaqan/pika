//! Local end-to-end call test: FfiApp caller ↔ pikachat daemon (bot).
//!
//! Validates the full call signaling path using:
//! - A local Nostr relay for MLS signaling
//! - A locally spawned `pikachat daemon` as the bot/callee
...
//!
//! Requires:
//! - `pikachat` built from the workspace crate `cli/` (run `just e2e-local-pikachat-daemon`)
...
// --- JSONL helpers for pikachat daemon ---
...
impl DaemonHandle {
    fn spawn(relay_url: &str, state_dir: &str) -> Self {
        ...
        assert!(
            ...
            "pikachat binary not found at {bin}. Build it: just e2e-local-pikachat-daemon"
        );
...
fn run_pikachat_call_test(relay_url: &str, moq_url: &str) {
    ...
    if !std::path::Path::new(&bin).exists() {
        eprintln!("SKIP: pikachat binary not found at {bin}");
        eprintln!("Build it: just e2e-local-pikachat-daemon");
        return;
    }
    ...
    // Spawn pikachat daemon as the "bot".
...
fn call_with_local_pikachat_daemon() {
    if std::env::var("PIKA_E2E_LOCAL").unwrap_or_default().trim() != "1" {
        eprintln!("SKIP: set PIKA_E2E_LOCAL=1 to run local pikachat tests");
        return;
    }
    ...
    run_pikachat_call_test(&relay.url, &moq.url);
    drop(relay);
...
fn call_with_local_pikachat_daemon_primal() {
    ...
    run_pikachat_call_test("wss://relay.primal.net", &moq.url);
}
