...
How to set up and manage group chats with the Pikachat OpenClaw plugin.

## Overview

The Pikachat plugin supports multi-participant MLS group chats over Nostr. Your bot can:
...
- **Group chats** → Isolated session per group (`pikachat:<accountId>:<groupId>`)
...
4. Check logs: `journalctl -u openclaw.service | grep pikachat`
